...
/**
 * Resource for collection of items with URI and provided methods for content creation and reading.
 *
 * @author ELETAI
 * @version 1.0
...
public abstract class ServiceRegistryItemCollectionResource<T>
    ...
    /**
     * Creates and adds a new item into the collection. Returns URI to the created item resources.
     *
     * @param itemRepresentation Representation of the resource to be added into the collection
     */
    public String createItem(String collectionId, T itemRepresentation)
        ...
        if (this.get().equals(ServiceDataDescription.class)) {
            ServiceDataDescription dataDesc = (ServiceDataDescription) itemRepresentation;
            newItemId = serviceDescriptionDAO
                .insertNewDataDescription(collectionId,
                    dataDesc);
        }
    ...
    private void buildHATEOASLinks(List<T> retList, UriInfo uriInfo) {
        ...
        for (T dE : (List<T>) retList) {
            ...
            if (this.get().equals(ServiceDataDescription.class)) {
                itemContLink.setTarget(tempUriBuilder.path(
                    "/"
                    + ((ServiceDataDescription) dE)
                    .getDataDescriptionId()).build());
            }
            if (this.get().equals(TechnicalServiceDescription.class)) {
                itemContLink.setTarget(tempUriBuilder.path(
                    "/"
