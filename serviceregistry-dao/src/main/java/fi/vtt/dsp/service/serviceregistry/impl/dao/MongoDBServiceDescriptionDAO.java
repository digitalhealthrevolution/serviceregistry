...
public class MongoDBServiceDescriptionDAO implements ServiceDescriptionDAO {
    ...
    @Override
    public ServiceDataDescription findDataDescription(String serviceId, String dataDescId) throws
        DAOGeneralSystemFault,
        DAONotFoundFault {

        ServiceDataDescription retDataDescr = null;
        DBObject dbObj = null;
        try {
            if ((serviceId != null) && (dataDescId != null)) {
                BasicDBObject serviceRegistryEntryPointer = new BasicDBObject(
                    "_id", new ObjectId(serviceId));
                BasicDBObject arrayPointer = new BasicDBObject(
                    "dataDescriptionId", dataDescId);
                BasicDBObject arrayQuery = new BasicDBObject("$elemMatch",
                    arrayPointer);
                serviceRegistryEntryPointer.append(
                    "serviceDescription.serviceDataDescription",
                    arrayQuery);
                // Find out if service registry entry with service data
                // description exists already
                dbObj = this.serviceRegistryEntryColl
                    .findOne(serviceRegistryEntryPointer);
                ...
                if (dbObj != null) {
                    BasicDBList techDescList = (BasicDBList) ((BasicDBObject) dbObj
                        .get("serviceDescription"))
                        .get("serviceDataDescription");

                    for (Object techDesc : techDescList) {
                        if (((BasicDBObject) techDesc).get(
                            "dataDescriptionId").equals(
                                dataDescId)) {
                            // Matching data description found
                            retDataDescr = DAOUtils.convertDBObjToWSObjViaJSON(
                                (DBObject) techDesc,
                                ServiceDataDescription.class);
                            break;
                        }
                    }
                }
            ...
            LOGGER.log(Level.SEVERE, "Error finding service data description.", e);
            throw new DAOGeneralSystemFault(
                "Failure while retrieving service data description "
                + dataDescId
                + " in service registry entry " + serviceId, e);
        } catch (IllegalArgumentException e) {
            throw new DAOGeneralSystemFault(
    ...
    @Override
    public String insertNewDataDescription(String serviceId, ServiceDataDescription techServDesc) throws
        DAOUpdateFailedFault,
        DAOGeneralSystemFault,
        DAONotSavedFault,
        DAONotFoundFault {

        String newDataDescriptionId = "";
        DBObject dbObj = null;
        ...
        try {
            ...
            BasicDBObject itemToBeInserted = new BasicDBObject(
                "dataDescriptionId",
                techServDesc.getDataDescriptionId());
            BasicDBObject arrayQuery = new BasicDBObject("$elemMatch",
                itemToBeInserted);
            serviceRegistryEntryPointer.append(
                "serviceDescription.serviceDataDescription",
            ...
            // If it does - do not insert service data description anymore
            if (dbObj != null) {
                throw new DAONotSavedFault(
                    "Service registry entry "
                    + serviceId
                    + " already has a service data description with id "
                    + techServDesc.getDataDescriptionId());
            }
            // If it doesn't - insert service data description

            BasicDBObject arrayInsertion = new BasicDBObject(
                "serviceDescription.serviceDataDescription",
                DAOUtils.convertWSObjToDBObjViaJSON(techServDesc,
                    ServiceDataDescription.class));
            WriteResult wr = this.serviceRegistryEntryColl.update(
                new BasicDBObject("_id", new ObjectId(serviceId)),
                new BasicDBObject("$addToSet", arrayInsertion));
            ...
            if (wr != null) {
                if (wr.getError() != null) {
                    throw new DAOUpdateFailedFault(
                        "Service data description was not inserted in the service registry entry "
                        + serviceId + " : " + wr.getError());
                }
                if (wr.getN() == 0) {
                    throw new DAONotFoundFault("Service registry entry id "
            ...
            updateModifiedOnDateInServiceDescription(serviceId);
            newDataDescriptionId = techServDesc.getDataDescriptionId();

        } catch (MongoException e) {
            LOGGER.log(Level.SEVERE,
                "Error inserting service data description.", e);
            throw new DAOGeneralSystemFault(
                "Failure inserting new service data description "
                + techServDesc.getDataDescriptionId()
                + " in service registry entry " + serviceId, e);
        } catch (IllegalArgumentException e) {
            throw new DAOGeneralSystemFault(
        ...
        return newDataDescriptionId;
    }

    @Override
    public String updateDataDescription(String serviceId, String dataDescId, ServiceDataDescription techServDesc) throws
        DAOGeneralSystemFault,
        DAOUpdateFailedFault,
        DAONotFoundFault {

        DBObject dbObj = null;
        String updatedDataDescriptionId = "";

        // FIXME: Maybe this one should be checked if same in the beginning
        techServDesc.setDataDescriptionId(dataDescId);
        ...
        try {
            ...
            if (dbObj == null) {
                throw new DAONotFoundFault("Service registry entry "
                    + serviceId + " with service data description "
                    + dataDescId + " to be updated was not found in DB");
            }
            ...
            BasicDBObject servRegEntryPointerWithArrayItem = serviceRegistryEntryPointer
                .append("serviceDescription.serviceDataDescription.dataDescriptionId",
                    dataDescId);
            ...
            if (wr != null && (wr.getError() != null || wr.getN() == 0)) {
                throw new DAOUpdateFailedFault("Service registry entry "
                    + serviceId + " with service data description "
                    + dataDescId + " update failed");
            }
            updateModifiedOnDateInServiceDescription(serviceId);
            updatedDataDescriptionId = techServDesc.getDataDescriptionId();
        ...
        return updatedDataDescriptionId;
    }

    @Override
    public String deleteDataDescription(String serviceId, String dataDescId) throws
        DAOUpdateFailedFault,
        DAOGeneralSystemFault,
        DAONotFoundFault {
        ...
        BasicDBObject arrayQuery = new BasicDBObject("$elemMatch",
            itemToBeDeletedPointer);
        serviceRegistryEntryPointer.append(
            "serviceDescription.serviceDataDescription", arrayQuery);
        ...
        try {
            ...
            if (wr != null) {
                if (wr.getError() != null) {
                    throw new DAOUpdateFailedFault(
                        "Service data description was not deleted in service registry entry "
                        + serviceId + " : " + wr.getError());
                }
                if (wr.getN() == 0) {
                    throw new DAONotFoundFault("Service data description "
                        + dataDescId + " or service registry entry "
                        + serviceId + " was not found");
                }
            ...
            throw new DAOGeneralSystemFault(
                "Failure deleting service data description "
                + dataDescId + " in service registry entry "
                + serviceId + " : " + e.getMessage(), e);
        }
        return retString;
    }
